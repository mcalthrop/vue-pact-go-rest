...
import (
	...
	"github.com/mcalthrop/vue-pact-go-rest/api/internal/model"
)
...
func (r *StaticRecipeRepository) ListRecipes() ([]model.RecipeSummary, error) {
	summaries := make([]model.RecipeSummary, len(r.recipes))
	for i, recipe := range r.recipes {
		summaries[i] = recipe.RecipeSummary
	}
...
func seedRecipes() []model.Recipe {
	return []model.Recipe{
		{
			RecipeSummary: model.RecipeSummary{
				ID:       "sourdough-boule",
